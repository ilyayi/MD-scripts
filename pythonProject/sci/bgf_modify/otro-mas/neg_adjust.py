def replace_multiple_columns_fixed_width1(source_path, target_path, columns, start_row, end_row, widths):
    ...
    for i, (src_line, tgt_line) in enumerate(
        ...
        if len(tgt_parts) > 12 and tgt_parts[12].startswith('-'):
            current_widths = [11, 5, 4, 4, 6, 3, 10, 10, 9, 7, 2, 2, 10]
            print('itt made it')
        else:
...
source_file = 'nik-step-done.bgf'
target_file = 'nik-step-done.bgf'
columns_to_replace = []
column_widths = [11, 5, 4, 4, 6, 3, 10, 10, 9, 7, 2, 3, 10]  # Initial column widths
...
def replace_multiple_columns_fixed_width1(source_path, target_path, columns, start_row, end_row, widths):
    ...
    for i, (src_line, tgt_line) in enumerate(
        ...
        # Check if the value in column 12 is negative and adjust the column widths accordingly
        if len(tgt_parts) > 12 and tgt_parts[12].startswith('-'):
            current_widths = [10, 6, 4, 4, 6, 3, 10, 10, 9, 7, 2, 2, 10]
            print('itt made it')
        else:
            current_widths = widths
...
target_file = 'nik-step-done.bgf'
columns_to_replace = []
column_widths = [10, 6, 4, 4, 6, 3, 10, 10, 9, 7, 2, 3, 10]  # Initial column widths

replace_multiple_columns_fixed_width1(source_file, target_file, columns_to_replace, 22, 97, column_widths)
